...
exports.saveImage = function(req, res) {
  ...
  revue.save(function(err) {
    if (err) {
      ...
      if (req.files.image.originalname !== null) {
        saveImageToServer(req, res, revue, true);
      } else {
        // res.jsonp(revue);
        res.setHeader('Content-Type', 'text/html');
        res.status(200);
    ...
    if (err) {
      ...
      revue.save(function(err) {
        if (err) {
          ...
          if (req.files.image.originalname !== null) {
            saveImageToServer(req, res, revue, false);
          } else {
            res.setHeader('Content-Type', 'text/html');
            // res.jsonp(revue);
            res.redirect('/#!/revues/' + revue._id);
...
exports.all = function(req, res) {
  Revue.find(req.query).sort({
    'ref': 1 / -1
  }).populate('emprunt.user', 'name').exec(function(err, revues) {
    if (err) {
      res.render('error', {
        status: 500
      });
...
exports.getMaxRef = function(req, res) {
  Revue.find().sort({
    'ref': 1 / -1
  }).limit(-1).exec(function(err, revue) {
    if (err) {
      res.render('error', {
        status: 500
      });
