...
function saveImageToServer(req, res, dvd, creation) {
...
exports.saveImage = function(req, res) {
  ...
  dvd.save(function(err) {
    if (err) {
      ...
      if (req.files.image.originalname !== null) {
        saveImageToServer(req, res, dvd, true);
      } else {
        // res.jsonp(revue);
        res.setHeader('Content-Type', 'text/html');
        res.status(200);
    ...
    if (err) {
      ...
      dvd.save(function(err) {
        if (err) {
          ...
          if (req.files.image.originalname !== null) {
            saveImageToServer(req, res, dvd, false);
          } else {
            res.setHeader('Content-Type', 'text/html');
            // res.jsonp(dvd);
            res.redirect('/#!/dvds/' + dvd._id);
...
exports.all = function(req, res) {
  Dvd.find(req.query).sort({
    'ref': 1 / -1
  }).populate('emprunt.user', 'name').exec(function(err, dvds) {
    if (err) {
      res.render('error', {
        status: 500
      });
