...
function saveImageToServer(req, res, livre, creation) {
    ...
    if (err) {
      ...
      livre.save(function(err) {
        if (err) {
          ...
          if (req.files.image.originalname !== null) {
            saveImageToServer(req, res, livre, false);
          } else {
            res.setHeader('Content-Type', 'text/html');
            // res.jsonp(livre);
            res.redirect('/#!/livres/' + livre._id);
...
exports.all = function(req, res) {
  Livre.find(req.query).sort({
    'ref': 1 / -1
  }).populate('emprunt.user', 'name').exec(function(err, livres) {
    if (err) {
      res.render('error', {
        status: 500,
        error: err
...
exports.getMaxRef = function(req, res) {
  Livre.find().sort({
    'ref': 1 / -1
  }).limit(-1).exec(function(err, livre) {
    if (err) {
      res.render('error', {
        status: 500
      });
